...
class Controller_Banner_Newscarousel extends Controller_Banner_Base
{


    protected function _getModule()
    {
        return Model_Banner::MODULE_NEWS_HOME_CAROUSEL;
    }
    ...
    public function indexAction()
    {
        ...
        if ($aList) {
            foreach ($aList as $key => $value) {
                ...
                $aModifyUser = Model_User::getDetail($value['iUpdateUserID']);
                $aList[$key]['sPublishUserName'] = $aPublishUser['sUserName'];
                $aList[$key]['sModifyUserName'] = $aModifyUser['sUserName'];
            }
        }
    ...
    public function offlineAction()
    {
        ...
        if ($aDetail) {
            ...
            $iCnt = Model_Banner::getCnt(['where' => ['iStatus' => 1, 'iPublishStatus' => 1, 'iModuleID' => $iModuleID, 'iTypeID' => $aDetail['iTypeID']]]);
    ...
    protected function _checkData()
    {
        ...
        if (empty($iNewsID)) {
            return $this->showMsg('请填写文章ID', false);
        }

        $aNews = Model_News::getPublishNewsDetail($iNewsID);
        if (empty($aNews)) {
            return $this->showMsg('所选文章暂未发布', false);
        }
    ...
    {
        $this->assign('sListUrl', '/banner/newscarousel/');
        $this->assign('sOfflineUrl', '/banner/newscarousel/offline/');
        $this->assign('sAddUrl', '/banner/newscarousel/add/');
        $this->assign('sEditUrl', '/banner/newscarousel/edit/');
        $this->assign('sPreviewUrl', '/banner/newscarousel/preview/');
