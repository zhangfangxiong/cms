...
class Model_Menu extends Model_Base
{
    ...
    public static function getMenus ($iProjectID, $iUserID = 0)
    {
        $aTree = self::getTree($iProjectID, $iUserID);
        $aList = array();
    ...
    public static function getTree ($iProjectID, $iUserID = 0)
    {
        $aWhere = array(
            'iStatus' => 1,
        ...
        // 权限判断
        if ($iUserID > 0) {
            $aMenuPermission = Model_Permission::getMenuPermissions($iUserID);
        }
        ...
        foreach ($aList as $aMenu) {
            ...
            $aOrder[] = $aMenu['iOrder'];
            unset($aMenu['iOrder'], $aMenu['iStatus'], $aMenu['iCreateTime'], $aMenu['iUpdateTime'], $aMenu['iProjectID']);
            $aData[] = $aMenu;
        }
        unset($aList);
