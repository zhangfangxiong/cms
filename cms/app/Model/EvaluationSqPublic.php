 ...
 * author: cjj
 */
class Model_EvaluationSqPublic extends Model_Base
{

    const DB_NAME = 'cms';

    const TABLE_NAME = 't_evaluation_sq_public';

    const CHAPTER = 8; //  当前章节ID
    /*
    * 获取所需配置
    *
    */
    ...
    public  static  function saveData($aParam)
    {
        ...
        $result = self::getSqPublicByEID($aParam['iEvaluationID']);
        $aParam['iStatus'] = 1;
        //获取开关配置
        $evaluationSwitch = self::getConfig();
        //如果开关打开就执行保存和更新操作
        if($evaluationSwitch['evaluationSwitch']) {
            if (!empty($result)) {
     ...
     *
    */
    public static function getSqPublicByEID($evaluationID) {
