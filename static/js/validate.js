...
(function($) {
	...
	$.validator.tipHandler = function(e, status, msg) {
		var tipele = $(e).nextAll('.validate_checktip:first')
		if (tipele.length == 0) {
			tipele = $(e).parents('[validate]').nextAll('.validate_checktip:first');
		}
		if (status == 2) {
			tipele.html('');
			tipele.removeClass('validate_wrong');
			tipele.removeClass('validate_right');
		} else if (status == 1) {
			tipele.removeClass('validate_wrong');
			tipele.addClass('validate_right');
			tipele.html(msg);
		} else {
			tipele.addClass('validate_wrong');
			tipele.removeClass('validate_right');
			tipele.html(msg);
		}
	}
